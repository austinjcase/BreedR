...
i<-2
  ...
  anova<-anova(out)
  anova
  number_reps<-length(unique(data3$rep))
  ...
  #qnorm(pval) *MSEline:rep * sqrt(1/reps + 1/reps)
  #qnorm(p=0.975) * sqrt((69.169*((1/3) + (1/3))))  ## 3 = reps and 69 is MSE error #13.30939
  #qnorm(p=0.975) * sqrt((69.169*((1/3) )))  ## 3 = reps and 69 is MSE error #13.30939

  means<-aggregate(yield ~ line ,FUN=mean, data=data3, na.rm=F, na.omit =F)
  trial_mean<-summary(data3$yield)[4]
  final<-rbind(means, data.frame(line=c("lsd_05", "lsd_10", "mean"), yield=c(lsd_05, lsd_10, trial_mean) ))
  final$trial_name<-rep(unique(data2$trial_name), nrow(final))
  out<-final
  rm(final)

  boot<-rbind(out, boot)

  one.year.single.locs<-boot
  rm(boot)

#####

# one year state wide average's multiple locatios
#####

  if (is.null (yr1)){ one.year<-NULL}
  sel<-c(yr1)
  data<-subset(vt1, year %in% sel) # filter by year
  ...
  final<-rbind(means, data.frame(line=c("lsd_05", "lsd_10", "mean"), yield=c(lsd_05, lsd_10, trial_mean) ))
  final$trial_name<- rep(paste(c("state_wide",sel), collapse = '_'), nrow(final))

  one.year<-final
  rm(final)
#####

# two year state  average's singel  locatios
#####

  if (is.null (yr2)){ two.year<-NULL}
  sel<-c(yr1,yr2)
  data<-subset(vt1, year %in% sel) # filter by year
  data<-droplevels(data)
  table(data$trial_name)
...
# two year state wide average's multiple locatios
#####
...
# three year state wide average's multiple locatios
#####

if (is.null (yr3)){ three.year<-NULL}
sel<-c(yr1,yr2,yr3)
data<-subset(vt1, year %in% sel) # filter by year
data<-droplevels(data)
table(data$trial_name)
...
xc[1]
xc<-as.vector(xc)
xc<-paste(xc, collapse = '_')
c("state_wide_",xc)


paste(c("state_wide",sel), collapse = '_')
...
gsub(" ", "-", xc)
  ...
  rep(paste(c("state_wide",sel)), nrow(final))


c("state_wide",sel)
xc<-paste(1:12, sep="", collapse=_)
xc
