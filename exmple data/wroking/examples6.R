...
for (i in 1:length(locs)) {
  ...
  q<-LSD.test(out, "line", 52, 1161.74,0.1)
  q

  q<-NULL

  out<-aov(trait ~ line  , data=data2) #aov
  anova(out)
  ...
  ?LSD.test()
  str(out)
  q<-LSD.test(out, "line", 54, 70.48    ,0.05)
  q
  ?qnorm()
  str(data2)
...
for (i in 1:length(locs)) {
  ...
  out<-aov(trait ~ line %in% rep * year  , data=data3) #aov with rep nexted in year
  anova(out)
  ...
  out2<-aov(trait ~ line * rep %in% year  , data=data3) #aov with rep nexted in year
  out2
  anova(out2)

  out<-aov(trait ~ line * year  , data=data3) #aov with rep nexted in year
  anova(out)
