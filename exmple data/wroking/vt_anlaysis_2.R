...
check<-subset(final_final, line != "lsd_05" & line != "lsd_10" & line != "mean" )
unique(check$line)

boxplot(check$trait)
min(check$trait, na.rm=T)
max(check$trait, na.rm=T)

names(final_final)[2]<-trait # rename trait to user spec for trait

head(final_final)
head(result_yr1)
head(result_yr1_state)
 head( result_yr2)
head(result_yr2_state)
head( result_yr2b)
 head(result_yr2_b_state)
head(  result_yr2c)
 head(result_yr2_c_state)
  head(  result_yr3)
 head(result_yr3_state )
 unique(final_final$trial_name)
